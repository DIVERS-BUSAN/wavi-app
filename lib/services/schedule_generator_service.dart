...
class ScheduleGeneratorService {
  ...
  Future<List<Schedule>> generateTravelItinerary({
    ...
    try {
      // 1. 시간 제약 계산
      Duration dailyAvailableTime = _calculateDailyAvailableTime(startTime, endTime);
  ...
  Future<List<Schedule>> _generateDaySchedule({
    ...
    for (int i = 0; i < places.length; i++) {
      ...
      // 이동시간만큼 시간 추가
      currentTime = currentTime.add(Duration(minutes: travelDuration));
  ...
  ScheduleColor _getColorForCategory(String category) {
    ...
    if (lowerCategory.contains('음식') || lowerCategory.contains('식당') || 
        lowerCategory.contains('카페') || lowerCategory.contains('레스토랑')) {
      return ScheduleColor.orange;
    } else if (lowerCategory.contains('관광') || lowerCategory.contains('명소') ||
               lowerCategory.contains('박물관') || lowerCategory.contains('미술관')) {
      return ScheduleColor.blue;
    } else if (lowerCategory.contains('쇼핑') || lowerCategory.contains('마트') ||
               lowerCategory.contains('백화점')) {
      return ScheduleColor.purple;
    } else if (lowerCategory.contains('자연') || lowerCategory.contains('공원') ||
               lowerCategory.contains('해변') || lowerCategory.contains('산')) {
  ...
  Future<List<Schedule>> _checkScheduleConflicts(
    ...
    List<Schedule> existingSchedules = allSchedules.where((schedule) {
      return schedule.dateTime.isAfter(startDate.subtract(const Duration(days: 1))) &&
  ...
  bool _schedulesOverlap(Schedule schedule1, Schedule schedule2) {
    ...
    return schedule1.dateTime.isBefore(end2) && end1.isAfter(schedule2.dateTime);
  }
}
